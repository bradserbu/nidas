/*
 * 	sample program to send Bisync data
 *
...
int main(int argc, char* argv[])
{
	...
	// bisync requires us to manually configure a message protocol
	buf = malloc(size + 2);
	if (!buf) {
	...
	params.syncflag = 0x9867;
	params.sixbitflag = 0;
	params.crctype = SEAMAC_CRC_NONE;

	// NOTE: allow extra time for final character + 2 closing flag
	params.posttxdelay = (1000 * 8 * 3) / params.rate; 
